 ...
 */
// Ran /pipeline first and last for a stretch (run 33253332263, DIA-201) to
// rule out a same-browser position bias — the theory that its 99 came from
// running third in a warm browser rather than from having no avatar to draw.
// Both positions scored 99. No bias found, so back to one pass per route: the
// 11-13 point gap on `/` and `/interview` is a Rive cost, not a script
// artifact.
