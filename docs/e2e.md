...
| Localization | `localization.spec.ts` | mid-interview switch stops and re-speaks; whole transcript re-renders; persona keeps its gender across the switch; ten screens fully translated with no raw keys; session speaks Ukrainian in both senses |
...
| Pro gate | `pro.spec.ts` | the two paid pills marked and pointed at the plans; the paid URLs closed, not just the links; Back leaves the gate rather than bouncing off it; the prices carry the "payments are not live" notice; a seeded entitlement opens both screens and unlocks the pills; Developers and the build version in the footer; the engine check offered as advice |
| Idle avatars | `avatars.spec.ts` | Marcus frozen with his eyes open; the same frozen frame however long the tile was hovered; the other three rigs still parked on the frame they load with. Measured off the canvas's own eye band, against the extremes of the rig's own blink — no hardcoded luminance and no screenshot baseline |
| Responsive & a11y | `responsive.spec.ts` | no horizontal scroll on nine routes × six widths; persona grid inside a 320px phone; five pipeline stages plus the offer on phone/tablet/desktop; 44px toolbar targets with the stubs hidden; Back/Home never over the language switcher; captions and mic named, focusable and keyboard-operable; AA contrast on every run of text, and on all three stage-card states |
