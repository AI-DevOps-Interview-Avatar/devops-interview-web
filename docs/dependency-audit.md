...
That is what the ticket asked for, and it does not work here. The one advisory
this project carries has **no fixed release to move to**, so the command would
fail every build from now until upstream ships one — and a gate that is red for
...
| `react-router` | GHSA-qwww-vcr4-c8h2 (RSC mode CSRF bypass) | **Allowlisted until 2026-10-31** — see below |
